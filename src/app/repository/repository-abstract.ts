...
export class RepositoryAbstract<entity, entityDTO> implements IRepository<entity,entityDTO>{
    ...
    async obterTodos(filter?: FilterQuery<entity>, sortColumn?: string): Promise<{ dados: entity[]; quantidade: number; }> {
        if(filter && filter.id){
            (filter as any)._id = filter.id;
            delete filter.id;
        }
        
        const dados = ((await this.entityModel.find(filter).sort(sortColumn)) as any[]).map((e)=>e.toJSON());
